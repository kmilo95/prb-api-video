...
original_author="kmilo95"
original_name="prb_api_video"
original_urlname="prb-api-video"
original_description="Awesome prb_api_video created by kmilo95" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/prb_api_video "${name}"
...
then
    ...
    echo "then run 'prb_api_video' or 'python -m prb_api_video'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'prb_api_video' or 'python -m prb_api_video' or 'poetry run prb_api_video'"
fi
